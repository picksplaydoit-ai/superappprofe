...
export const StudentsModule: React.FC<StudentsModuleProps> = ({ course, onUpdate }) => {
  ...
  const processNamesAndAdd = (names: string[]) => {
    ...
    // 2. Eliminar duplicados dentro del mismo bloque
  ...
  const handleCSVImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Procesamiento simple de CSV: Una línea por registro, tomamos la primera columna
        const lines = text.split(/\r?\n/);
        const names = lines
          .map(line => {
            // Manejar posibles comillas en el CSV
  ...
  const confirmDeleteStudent = (id: string, name: string) => {
    setModalConfig({
      ...
      message: 'Se borrarán todas sus asistencias y calificaciones registradas en este curso.'
    });
  };
        ...
        <div className="mb-8 bg-white p-6 rounded-3xl border-2 border-indigo-100 animate-in zoom-in-95 duration-300 shadow-xl shadow-indigo-50/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 border-dashed flex flex-col items-center text-center">
              ...
              <h4 className="font-bold text-slate-800 text-sm mb-2">Archivo CSV</h4>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight mb-5 leading-relaxed">Sube tu lista desde un archivo .csv</p>
              ...
              <button 
                ...
                className="w-full bg-white border-2 border-indigo-600 text-indigo-600 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 transition-all flex items-center justify-center gap-2"
              >
                {isProcessing ? <div className="w-4 h-4 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin" /> : <UploadCloud size={16} />}
                {isProcessing ? 'Procesando...' : 'Seleccionar .CSV'}
              </button>
            </div>
